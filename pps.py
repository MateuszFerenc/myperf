...
def analyze_perf_script(ignore, tracepoints, data):
	...
	for i in range(len(lines)):
		...
		if event_ in ignore:
			ignored += 1
			continue
		values.append(round(time_ * 1000, 4))
	_min = round(min(values), 2)
	_max = round(max(values), 2)
	_avg = round(mean(values), 2)
	_stdev = round(stdev(values), 2)
	
	return (_min, _max, _avg, _stdev, values, ignored)
...
if __name__ == "__main__":
	...
	if args.tracepoints is not None and args.file is not None:
		with open(args.file, "r") as file_data:
			data = file_data.read()
		result = analyze_perf_script(ignore=(list(args.ignore.split(" ")) if args.ignore is not None else []), tracepoints=list(args.tracepoints.split(" ")), data=data)
		print(f"Minumum: {result[0]} ms\nMaximum: {result[1]} ms\nAverage: {result[2]} ms\nStandard deviation: {result[3]} ms\nIgnored tracepoints: {result[5]}")
		plotter.plot(result[4], color='r', label='funcB')
		plotter.xticks(range(0, len(result[4])))
		plotter.xlabel('tracepoint no.', weight='light', style='italic')
		plotter.ylabel('time [ms]', weight='light', style='italic')
		plotter.title(f"{args.ignore} {args.tracepoints} exectution time", weight='bold')
		plotter.legend()
